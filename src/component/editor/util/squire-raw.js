...
//4146行有修改   innerText =>innerHTML
//2546行有修改    true => 变成contentEditable
//4150-4152行加入    为了插入Excel表格
...
// save an undo state and hope for the best.
    ...
    function Squire(root, config) {
        ...
        /******** 有修改 *********/
        // root.setAttribute('contenteditable', config.contentEditable);
        root.setAttribute('contenteditable', config.contentEditable);
...
// --- Get/Set data ---
    ...
    proto.setHTML = function (html) {
        ...
        // html = html.replace(/\n/ig, '');
        // html = html.replace(/<\/{0,1}[^(p|img)].*?>/ig, '');
        // html = html.replace(/<p.*?>/ig, '<p>');
        // //替换图片。com到cn
        // html = html.replace(/resource\.handsight\.com\/TVHelp01/ig, 'resource.handsight.cn/TVHelp01');
    ...
    proto.insertHTML = function (html, isPaste) {
        ...
        if (typeof DOMPurify !== 'undefined' && DOMPurify.isSupported) {
            ...
            if (isPaste) {
                ...
                if(html.indexOf('<table')==-1 && html.indexOf('<tr')!==-1){
                    html = `<table border='1px solid grey' cellpadding=0 cellspacing=0 style='border-collapse:collapse;'>${html}</table>`
            ...
            //原本是 var text = newDom.body.innerText;
            var text = newDom.body.innerHTML;
            // console.log('text',text);
            // html = html.replace(/<svg.*?>.*<\/svg>/ig,"");
            // // 替换行内元素前后
            // html = html.replace(/<(\/|a|span|b|i|strong|code).*?>/ig,"");
            // // 替换块级元素后面
            // html = html.replace(/<\/[^(img)].*?>/ig,"</p>");
            // // 替换块级元素前面
            // html = html.replace(/<[^(\/img)].*?>/ig,"<p>");
